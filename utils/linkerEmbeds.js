...
export function errEmbed(text) {
  return new EmbedBuilder()
    .setColor(FUSION_DARK)
    .setDescription(`❌ ${text}`);
}

export function playerInfoEmbed(data) {
  if (!data || !data.guild) {
    throw new Error('Missing required data for player info embed');
  }
  ...
  const ranksValue = data.ranks && data.ranks.length > 0
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '🔴 Donator ranks:', value: ranksValue }
    )
